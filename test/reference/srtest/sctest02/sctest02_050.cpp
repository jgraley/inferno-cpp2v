...
wait(SC_ZERO_TIME);
{
state=((!(! ::Multiplier::instigate)) ? (lmap[ ::Multiplier::T_STATE_PROCEED_THEN_ELSE]) : (lmap[ ::Multiplier::T_STATE_PROCEED_NEXT1]));
goto *(state);
}
PROCEED_NEXT:;
wait(SC_ZERO_TIME);
{
state=((! ::Multiplier::instigate) ? (lmap[ ::Multiplier::T_STATE_PROCEED_NEXT1]) : (lmap[ ::Multiplier::T_STATE_PROCEED_THEN_ELSE]));
goto *(state);
}
PROCEED_THEN_ELSE:;
...
{
state=((!(! ::Multiplier::proceed)) ? (lmap[ ::Multiplier::T_STATE_PROCEED_THEN_ELSE1]) : (lmap[ ::Multiplier::T_STATE_PROCEED_NEXT2]));
goto *(state);
}
PROCEED_NEXT1:;
wait(SC_ZERO_TIME);
{
state=((! ::Multiplier::proceed) ? (lmap[ ::Multiplier::T_STATE_PROCEED_NEXT2]) : (lmap[ ::Multiplier::T_STATE_PROCEED_THEN_ELSE1]));
goto *(state);
}
PROCEED_THEN_ELSE1:;
