#include "isystemc.h"

class TopLevel;
class TopLevel : public sc_module
{
public:
SC_CTOR( TopLevel )
{
SC_METHOD(T);
}
/*temp*/ int temp_j;
/*temp*/ int f_return;
/*temp*/ int result;
/*temp*/ int f_i;
/*temp*/ int result1;
/*temp*/ int result2;
int x;
/*temp*/ char f_k;
private:
int i;
public:
/*temp*/ int result3;
private:
unsigned int link;
public:
/*temp*/ int temp_j1;
/*temp*/ unsigned int temp_link;
enum TStates
{
T_STATE_LINK = 1U,
T_STATE_ENTER_f = 3U,
T_STATE_LINK1 = 0U,
T_STATE_LINK2 = 2U,
};
/*temp*/ int result4;
/*temp*/ int temp_i;
/*temp*/ unsigned int f_link;
/*temp*/ int result5;
/*temp*/ int result6;
/*temp*/ int result7;
private:
short j;
public:
/*temp*/ int temp_k;
/*temp*/ int temp_k1;
/*temp*/ int temp_j2;
/*temp*/ int result8;
/*temp*/ int temp_i1;
/*temp*/ int temp_i2;
/*temp*/ int result9;
void T();
/*temp*/ int result10;
private:
int t;
public:
/*temp*/ int temp_k2;
/*temp*/ int result11;
private:
char k;
public:
/*temp*/ short f_j;
private:
unsigned int state;
};
TopLevel top_level("top_level");

void TopLevel::T()
{
/*temp*/ bool enabled = true;
if( (0U)==(sc_delta_count()) )
{
 ::TopLevel::x=(0);
 ::TopLevel::temp_k=(8);
 ::TopLevel::temp_j2=(6);
 ::TopLevel::temp_j1=(0);
 ::TopLevel::temp_i2=(0);
 ::TopLevel::temp_k1=(0);
 ::TopLevel::f_i= ::TopLevel::temp_i2;
 ::TopLevel::f_j= ::TopLevel::temp_j1;
 ::TopLevel::f_k= ::TopLevel::temp_k1;
 ::TopLevel::f_link= ::TopLevel::T_STATE_LINK1;
next_trigger(SC_ZERO_TIME);
 ::TopLevel::state= ::TopLevel::T_STATE_ENTER_f;
enabled=(false);
}
if( enabled&&( ::TopLevel::state== ::TopLevel::T_STATE_LINK1) )
{
 ::TopLevel::result5= ::TopLevel::f_return;
 ::TopLevel::result7= ::TopLevel::result5;
 ::TopLevel::result= ::TopLevel::result7;
 ::TopLevel::result11= ::TopLevel::result;
 ::TopLevel::temp_i= ::TopLevel::result11;
 ::TopLevel::f_i= ::TopLevel::temp_i;
 ::TopLevel::f_j= ::TopLevel::temp_j2;
 ::TopLevel::f_k= ::TopLevel::temp_k;
 ::TopLevel::f_link= ::TopLevel::T_STATE_LINK;
 ::TopLevel::state= ::TopLevel::T_STATE_ENTER_f;
}
if( enabled&&( ::TopLevel::T_STATE_LINK== ::TopLevel::state) )
{
 ::TopLevel::result3= ::TopLevel::f_return;
 ::TopLevel::result4= ::TopLevel::result3;
 ::TopLevel::result2= ::TopLevel::result4;
 ::TopLevel::result9= ::TopLevel::result2;
 ::TopLevel::x= ::TopLevel::result9;
 ::TopLevel::temp_i1=(1);
 ::TopLevel::temp_k2=(3);
 ::TopLevel::temp_j=(2);
 ::TopLevel::f_i= ::TopLevel::temp_i1;
 ::TopLevel::f_j= ::TopLevel::temp_j;
 ::TopLevel::f_k= ::TopLevel::temp_k2;
 ::TopLevel::f_link= ::TopLevel::T_STATE_LINK2;
 ::TopLevel::state= ::TopLevel::T_STATE_ENTER_f;
}
if( enabled&&( ::TopLevel::state== ::TopLevel::T_STATE_LINK2) )
{
 ::TopLevel::result1= ::TopLevel::f_return;
 ::TopLevel::result10= ::TopLevel::result1;
 ::TopLevel::result8= ::TopLevel::result10;
 ::TopLevel::result6= ::TopLevel::result8;
cease(  ::TopLevel::x+((2)* ::TopLevel::result6) );
enabled=(false);
if( enabled )
 ::TopLevel::state= ::TopLevel::T_STATE_ENTER_f;
}
if( enabled&&( ::TopLevel::state== ::TopLevel::T_STATE_ENTER_f) )
{
 ::TopLevel::link= ::TopLevel::f_link;
 ::TopLevel::k= ::TopLevel::f_k;
 ::TopLevel::j= ::TopLevel::f_j;
 ::TopLevel::i= ::TopLevel::f_i;
 ::TopLevel::t=(( ::TopLevel::j*(3))+ ::TopLevel::i);
 ::TopLevel::f_return=( ::TopLevel::t+((5)* ::TopLevel::k));
 ::TopLevel::temp_link= ::TopLevel::link;
 ::TopLevel::state= ::TopLevel::temp_link;
}
if( enabled )
next_trigger(SC_ZERO_TIME);
}
