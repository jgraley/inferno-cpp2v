...
{    
        ...
        if( ContainerInterface *under_container = dynamic_cast<ContainerInterface *>(under_items[i]) )                
        {
            ...
            else
            {
                ...
                for( const TreePtrInterface &under_elt : *under_container )
                {
                    ...
                    child_patches.push_back( make_shared<TreeZonePatch>(make_unique<XTreeZone>(under_zone)) );        
                }
            }
        }            
    ...
    return make_shared<FreeZonePatch>( make_unique<FreeZone>(zone), move(child_patches) );         
}
