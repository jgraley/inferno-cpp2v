...
IsMemberOperator::IsMemberOperator( shared_ptr<SymbolExpression> a_, 
                                    shared_ptr<SymbolExpression> b_ ) :
    a(a_),
    b(b_)
{
...
IsMemberOperator *IsMemberOperator::Clone() const
{
    return new IsMemberOperator( a, b );
}
    

list<shared_ptr<SymbolExpression> *> IsMemberOperator::GetSymbolOperandPointers()
{
    return { &a, &b };
}


unique_ptr<BooleanResult> IsMemberOperator::Evaluate( const EvalKit &kit,
                                                      list<unique_ptr<SymbolResultInterface>> &&op_results ) const 
{
    // Only need to check that a is singular; b is expected to be a set
    if( !op_results.front()->IsDefinedAndUnique() )
        return make_unique<BooleanResult>( false );

    unique_ptr<SymbolResultInterface> ra = move( op_results.front() );
    unique_ptr<SymbolResultInterface> rb = move( op_results.back() );

    // TODO don't extensionalise!!!
    set<SR::XLink> set_xlinks;
    bool res = rb->TryGetAsSetOfXLinks( set_xlinks );
    if( res )
        res = set_xlinks.count( ra->GetOnlyXLink() ) > 0;
    return make_unique<BooleanResult>( res );    
}


shared_ptr<SymbolExpression> IsMemberOperator::TrySolveFor( const SolveKit &kit, shared_ptr<SymbolVariable> target ) const
{  
    // If the singular a is the or can be solved for the target, will 
    // return the range b or a solution based on b. Otherwise will fail.
    return a->TrySolveForToEqual( kit, target, b );
}


bool IsMemberOperator::IsCommutative() const
{
    return false;
}


Transitivity IsMemberOperator::GetTransitivityWith( shared_ptr<PredicateOperator> other ) const
{
    // Unlikely because a is singlar and b is a set
    return Transitivity::NONE;
}


string IsMemberOperator::RenderNF() const
{
    return RenderForMe(a) + " ∈ " + RenderForMe(b);
}


Expression::Precedence IsMemberOperator::GetPrecedenceNF() const
{
