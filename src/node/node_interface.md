...
CodeUnit trees are networks of nodes (actually acyclic directed graphs). We do not attempt to place a tree within a single container object which could marshal the incoming method calls, because this approach is too inflexible - algorithms need to get "close" to the nodes themselves. So instead we expose a tree as a multiplicity of nodes and ensure that client code can move between nodes as required.
...
`order_property` should be checked: if `TOTAL` then a total ordering should be implemented, even if this requires comparison of pointers. If `REPEATABLE`, only repeatable comparisons should be included.
...
`Sequence` and `Collection` both support the `Mutate()` method. Given an iterator `i`, you can call `i.Mutate( p )` where `p` is a compatible `TreePtr<>`. In `Sequence`s, this is equivalent to `*i = p`, but in `Collection`s you cannot do this because `*i` is const, so you need `Mutate()` which does `erase()` and `insert()`. `Mutate()` is available in the common `ContainerInterface<>` interface.
