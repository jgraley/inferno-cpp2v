...
string ConjunctionAgent::GetAgentRender( VN::RendererInterface *renderer, Syntax::Production surround_prod, Syntax::Policy policy ) const
{
	(void)surround_prod;
	...
	list<string> ls;
	for( const TreePtrInterface &p : GetConjuncts() )                 
		ls.push_back( renderer->DoRenderPreserve( (TreePtr<Node>)p, Syntax::Production::VN_CONJUNCTION, policy ) );
	return Join(ls, " ∧ ");
}    
