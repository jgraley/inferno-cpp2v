...
Syntax::Production BuildIdentifierAgent::GetAgentProduction( const VN::RendererInterface *, Syntax::Policy ) const
{
...
string BuildIdentifierAgent::GetAgentRender( VN::RendererInterface *renderer, Syntax::Production surround_prod, Syntax::Policy policy ) const
{
	...
	for( TreePtrInterface &source : scopy )
		ls_sources.push_back( renderer->DoRender( &source, Syntax::Production::COMMA_SEP, policy ) );
