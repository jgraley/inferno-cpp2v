...
The function's body is a `Compound` so that we can couple into the replace pattern, preserving the function's existing body. We add a `Conjunction` here, with a conjunct that goes though a `Stuff` node to an `Local` node. Because we will remove this `Local` suring replace, this ensures the enclosing pattern does not spin, adding more and more stack indexes forever. It also means we do not modify functions that already _do not_ have automatic variables.
