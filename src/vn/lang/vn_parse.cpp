...
// Designation -> Coupling (because all wildcards in the pattern must match equally at all usages of the coupling)

// Namespaces: CPPTree should be assumed as a default where not specified. More than one specifier is still TBD
// Fix OnIdByName to allow other than CPPTree (even though it won't work yet)
